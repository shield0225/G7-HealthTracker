...
const SystolicBloodAverageChart = () => {
  ...
  const highestSystolicBloodPressureDataCurrentWeek =
    filterGroupedDataByWeekDates(
      highestSystolicBloodPressureByDate,
      currentWeekDates
    );
  const highestSystolicBloodPressureDataLastWeek = filterGroupedDataByWeekDates(
    highestSystolicBloodPressureByDate,
    lastWeekDates
  );

  const currentWeekAverage =
    highestSystolicBloodPressureDataCurrentWeek.reduce(
      (acc, item) => acc + item,
      0
    ) / 7;
  const lastWeekAverage =
    highestSystolicBloodPressureDataLastWeek.reduce(
      (acc, item) => acc + item,
      0
    ) / 7;
  const percentageHigherLastWeek = calculatePercentageHigher(
    currentWeekAverage,
    lastWeekAverage
  );
  ...
  const seriescolumnchart = [
    {
      name: "Current Week",
      color: secondary,
      data: highestSystolicBloodPressureDataCurrentWeek,
    },
    {
      name: "Last Week",
      color: "#FDBB3A",
      data: highestSystolicBloodPressureDataLastWeek,
    },
  ];
